...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('!entrenosotros'):
            params = message.content.split()
            team_size = int(params[1])
            champions = randomChamps(champs, team_size * 4)
            self._last_state = champions
            await message.channel.send(splitTeams(self._last_state))

        if message.content.startswith('!reroll'):
            params = message.content.split()
            new_champ = randomChamps(list(set(champs) - set(self._last_state)), 1)
            index = self._last_state.index(params[1])
            self._last_state[index] = new_champ[0]
            await message.channel.send(splitTeams(self._last_state))

        if message.content.startswith('!random'):
            params = message.content.split()[1:]
            await message.channel.send(random.sample(params,k=1))
